...
Historically, Subsquid SDK used a [Store interface](../../reference/store-interface.md) as an abstraction layer over the database. As explained in the dedicated reference page, this is relies on the `typeorm` library.
...
3. Launch the metadata explorer command (see [here](../../key-concepts/typegen.md#blockchain-metadata), or skip it and add the Archive URL in `typegen.json`)
4. Change the `typegen.json` configuration file as specified above
5. Launch the Typegen command to generate new interfaces (see [here](../../key-concepts/typegen.md#typescript-class-wrappers))
