...
- [Extension of the GraphQL API](/firesquid/graphql-api/custom-resolvers) with arbitrary SQL
- [Secret environment variables](/firesquid/deploy-squid/env-variables), allowing to seamlessly use private third-party JSON-RPC endpoints and integrate with external APIs
- [API versioning and aliasing](/firesquid/deploy-squid/promote-to-production)
- [API Caching](/firesquid/graphql-api/caching)  

For a full feature set comparison, see [Subsquid vs The Graph](/firesquid/migrate/subsquid-vs-thegraph).
...
A boilerplate code for decoding EVM logs and the contract access classes will be generated in `src/abi/Gravity.ts`. In particular, the file contains the topic definitions used at the next step.  For more details about the EVM typegen tool, read a [dedicated doc page](/firesquid/evm-indexing/squid-evm-typegen).
...
To index smart contract data deployed to other EVM chains like Polygon or Binance Smart Chain, simply change the `archive` endpoint. For the list of the supported networks and the configuration details, see [here](/firesquid/evm-indexing/configuration).
...
- Have a closer look at the `EvmBatchProcessor` batch-oriented [programming model](/firesquid/evm-indexing)
- Learn how to [deploy a squid to the Aquarium hosted service](/firesquid/deploy-squid) for free
- Learn how to [index and query the contract state](/firesquid/evm-indexing/query-state)
