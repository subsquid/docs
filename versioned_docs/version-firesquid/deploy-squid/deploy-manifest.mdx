...
The deployment manifest is named `squid.yaml` by convention. The manifest defines how the squid should be built, customized and scaled during the deployment to Aquarium. It is used together with [`sqd deploy`](/firesquid/squid-cli/deploy) to deploy or update an existing squid version.
...
to the default [folder structure](/firesquid/basics/squid-structure).
...
See [Troubleshooting](/firesquid/deploy-squid/troubleshooting/#local-build) for instructions on how to build and run the Docker image locally.
...
See the [Scale the deployment](/firesquid/deploy-squid/scale) section.
