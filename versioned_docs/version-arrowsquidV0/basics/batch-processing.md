...
- Use the [MakerDAO `Multicall` contract](/arrowsquid-docs-v0/evm-indexing/query-state/#batch-state-queries) to batch EVM state queries.
- Use [`XXX.getMany()`](/arrowsquid-docs-v0/substrate-indexing/storage-state-calls) to batch Substrate state queries.
...
An idiomatic use of [`processor.run()`](/arrowsquid-docs-v0/basics/squid-processor) is as follows:
