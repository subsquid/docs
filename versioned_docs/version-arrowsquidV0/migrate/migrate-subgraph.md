...
At the same time, concepts of the [schema file](/arrowsquid-docs-v0/store/postgres/schema-file), [code generation from the schema file](/arrowsquid-docs-v0/store/postgres/schema-file/intro/#typeorm-codegen) and [auto-generated GraphQL API](/arrowsquid-docs-v0/graphql-api) should be familiar to subgraph developers. In most cases the schema file of a subgraph can be imported into a squid as is. 

There are some known limitations:
- Many-to-Many entity relations should be [modeled explicitly](/arrowsquid-docs-v0/store/postgres/schema-file/entity-relations/#many-to-many-relations) as two many-to-one relations
...
- [Extension of the GraphQL API](/arrowsquid-docs-v0/graphql-api/custom-resolvers) with arbitrary SQL
- [Secret environment variables](/arrowsquid-docs-v0/deploy-squid/env-variables), allowing to seamlessly use private third-party JSON-RPC endpoints and integrate with external APIs
- [API versioning and aliasing](/arrowsquid-docs-v0/deploy-squid/promote-to-production)
- [API caching](/arrowsquid-docs-v0/graphql-api/caching)

For a full feature set comparison, see [Subsquid vs The Graph](/arrowsquid-docs-v0/migrate/subsquid-vs-thegraph).

## Squid setup 

### 1. Install Squid CLI

Instructions [here](/arrowsquid-docs-v0/squid-cli/installation).
...
Next, we generate the entities from the schema using the [`squid-typeorm-codegen`](/arrowsquid-docs-v0/store/postgres/schema-file/intro/#typeorm-codegen) tool of the Squid SDK, then build the squid:
```bash
sqd codegen
sqd build
```
...
After that, start the local database and generate migrations from the generated entities using the [`squid-typeorm-migration`](/arrowsquid-docs-v0/store/postgres/db-migrations) tool:
```bash
sqd up
...
Check out the [EVM indexing](/arrowsquid-docs-v0/evm-indexing) section for the list of supported networks and configuration details.
...
Next, we make a [batch handler](/arrowsquid-docs-v0/basics/squid-processor/#processorrun) collecting the updates from a single batch of EVM logs. To convert a `0x...` string into a byte array we use the `decodeHex` utility from Subsquid SDK.
...
- Have a closer look at [`EvmBatchProcessor`](/arrowsquid-docs-v0/evm-indexing)
- Learn how to [deploy a squid to Subsquid Cloud](/arrowsquid-docs-v0/deploy-squid) for free
- Learn how to [index and query the contract state](/arrowsquid-docs-v0/evm-indexing/query-state)
