...
File-based data formats like [CSV](/arrowsquid-docs-v0/tutorials/index-to-local-csv-files) are convenient for data analysis, especially in the early prototyping stages. However, when working with large datasets the ability to read data files partially is a common requirement. This is rarely possible with CSV.
...
The `Database` class gets its name from the interface that was originally developed to [access an actual database](/arrowsquid-docs-v0/store/postgres/typeorm-store). Here, the interface is [used without modification](/arrowsquid-docs-v0/store/custom-database) in a class designed to access a filesystem. `Table`s play a similar role to that of tables of an actual database: they represent collections of rows, all of which share same set of fields/columns. Each such data structure requires one or more data files to store it in both CSV and Parquet, hence the mapping of `Table`s to files.
...
There are two main differences from the [CSV tutorial](/arrowsquid-docs-v0/tutorials/index-to-local-csv-files) and the first one is that for this project we will be using a `Table` implementation from `@subsquid/file-store-parquet`. Let's install it:
