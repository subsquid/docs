...
Our [schema definition](/arrowsquid-docs-v0/store/postgres/schema-file) for modelling this data is straightforward:
...
```
Note:
* a one-to-many [relation](/arrowsquid-docs-v0/store/postgres/schema-file/entity-relations) between `Owner` and `Transfer`;
...
Finally, we create [database migrations](/arrowsquid-docs-v0/store/postgres/db-migrations) to match the changed schema. We restore the database to a clean state, then replace any existing migrations with the new one:
```bash
sqd down
sqd up
...
`SubstrateBatchProcessor`s [exposes methods](/arrowsquid-docs-v0/substrate-indexing/setup) to "subscribe" them to specific data such as Substrate events, extrinsics, storage items etc. The `Contracts` pallet emits `ContractEmitted` events wrapping the logs emitted by the WASM contracts. Processor [allows one](/arrowsquid-docs-v0/substrate-indexing/specialized/wasm) to subscribe to such events emitted by a specific contract.
...
Once requested, the events can be processed by calling the `.run()` function that starts generating requests to the Archive for [*batches*](/arrowsquid-docs-v0/basics/batch-processing) of data.
...
Finally, these [TypeORM entity](/arrowsquid-docs-v0/store/postgres/schema-file/entities) instances are saved to the database, all in one go. This is done to reduce the number of database queries.
