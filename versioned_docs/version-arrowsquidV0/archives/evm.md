...
Main URLs of EVM Archives are available on the [Supported networks page](/arrowsquid-docs-v0/evm-indexing/supported-networks/#raw-urls).
...
Suppose we want data on Ethereum txs to `vitalik.eth`/`0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045` from block 16_000_000. We begin by finding the main archive URL for Ethereum Mainnet on the [Supported networks page](/arrowsquid-docs-v0/evm-indexing/supported-networks/#raw-urls). Then we have to:
...
```
A log will be included in the response if it matches all the requests. An empty array matches no logs; omitted or `null` request matches any log. See [EVM logs](/arrowsquid-docs-v0/evm-indexing/configuration/evm-logs) for a detailed description of data request fields.
...
```
A transaction will be included in the response if it matches all the requests. An empty array matches no transactions; omitted or `null` request matches any transaction. See [EVM transactions](/arrowsquid-docs-v0/evm-indexing/configuration/transactions) for a detailed description of data request fields.
...
```
A trace will be included in the response if it matches all the requests. An empty array matches no traces; omitted or `null` request matches any trace. See [Traces](/arrowsquid-docs-v0/evm-indexing/configuration/traces) for a detailed description of data request fields.
...
```
A state diff will be included in the response if it matches all the requests. An empty array matches no state diffs; omitted or `undefined` request matches any state diff. See [Storage state diffs](/arrowsquid-docs-v0/evm-indexing/configuration/state-diffs) for a detailed description of data request fields.

## Data fields selector

A JSON selector of fields for the returned data items. Documented in the [Field selectors](/arrowsquid-docs-v0/evm-indexing/configuration/data-selection/#field-selectors) section.
