...
The deployment manifest is named `squid.yaml` by convention. The manifest defines how the squid should be built, customized and scaled during the deployment to Subsquid Cloud. It is used together with [`sqd deploy`](/arrowsquid-docs-v0/squid-cli/deploy) to deploy or update an existing squid version.
...
to the default [folder structure](/arrowsquid-docs-v0/basics/squid-structure).
...
See [Troubleshooting](/arrowsquid-docs-v0/deploy-squid/troubleshooting/#local-build) for instructions on how to build and run the Docker image locally.
...
See [Postgres add-on](../pg-addon).

- **`rpc`**

See [RPC proxy](../rpc-proxy).
...
See the [Scale the deployment](/arrowsquid-docs-v0/deploy-squid/scale) section.
