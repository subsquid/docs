...
- When the data source is a `ChainDataSource = {chain: ChainRpc}`, the processor will obtain data _only_ from a node RPC endpoint. This mode of operation is slow, but requires no archive and. Like `MixedDataSource`, it also has almost [no chain latency](/arrowsquid-docs-v0/basics/unfinalized-blocks). It can be used with Substrate networks not listed on the [supported networks](../../supported-networks) page and with local development nodes.
...
There a [mini-guide](/arrowsquid-docs-v0/troubleshooting/#where-do-i-get-a-type-bundle-for-my-chain) on how to obtain type bundles for Substrate chains without relying on Subsquid tools.
