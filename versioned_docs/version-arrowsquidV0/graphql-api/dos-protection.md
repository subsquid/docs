...
To enable the protection for squids deployed to Subsquid Cloud, add the corresponding flags the GraphQL `api` service command in the [deployment manifest](/arrowsquid-docs-v0/deploy-squid/deploy-manifest/#deploy). Here is an example:
...
Same as `--max-response-size` but for live query [subscriptions](/arrowsquid-docs-v0/graphql-api/subscriptions).
