# Deployment key and Authentication
...
To find out other important CLI commands for interacting with our SaaS solution are listed under the [dedicated subsection of complete CLI Reference](squid-cli/#subcommands-for-squid-command).

## Authentication

In order to accomplish any tasks related to hosted squids, the terminal window in which the commands are being launched, needs to be authenticated. To do so, use the command command:
