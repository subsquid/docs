...
There are currently two major Archive releases incompatible with each other: `v5` and `FireSquid`. The current docs cover `FireSquid` as the old `v5` archives are now considered deprecated and will be sunset. See [this page](/archives/new-in-fire-squid) for an overview of FireSquid features.
