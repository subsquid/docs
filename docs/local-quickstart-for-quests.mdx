...
We recommend configuring NodeJS to install global packages to a folder owned by an unprivileged accout. Create the folder by running
```bash
...
```
Make sure that any executables globally installed by NodeJS are in `PATH`. That allows running them from any terminal. Open the `~/.bash_profile` file in a text editor and add the following line at the end:
```
