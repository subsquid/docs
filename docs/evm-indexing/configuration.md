...
The `option` argument supports filtering by topic and data selectors to specify which data is provided by the corresponding [block item](/evm-indexing/context-interfaces):
...
Fetch `NewGravatar` and `UpdateGravatar` logs emitted by `0x2E645469f354BB4F5c8a05B3b30A929361cf77eC`. For each log, fetch the transaction input, the topic set and the log data:
...
:::info
Note that normally one can use human-readable topic definitions generated by [evm-typegen](/basics/typegen/squid-evm-typegen) from the contract ABI. So in practice, the above snippet would rather look like 
```ts
 //...
 .addLog('0x2E645469f354BB4F5c8a05B3b30A929361cf77eC', {
    filter: [[
...
```
The requested fields will be populated in the corresponding [block items](/evm-indexing/context-interfaces) of the `processor.run()` context.
