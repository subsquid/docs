...
In all cases typegen will use `basename` of the ABI as the root name for the generated files. You can overwrite the basename of generated files using the fragment `(#)` suffix.
...
processor.run(new TypeormDatabase(), async (ctx) => {
  for (let c of ctx.blocks) {
    ...
    let supply = await (new Contract(ctx, blockHeader, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2').totalSupply())
  }
});
```
