...
Here,
* `https://v2.archive.subsquid.io/network/fuel-testnet` is the address for the public Subsquid Network gateway for Fuel testnet. Check out [Subsquid Networks reference pages](/subsquid-network/reference) for lists of public gateways for all supported networks.
* The argument of [`addReceipt()`](/fuel-indexing/fuel-datasource/receipt) is a set of filters that tells the processor to retrieve all receipts of type `LOG`.
