...
:::info
Token requirement has been raised from 10,000 to 1,000,000 SQD in Q1 2026.
:::
...
* 1,000,000 or more `SQD` tokens
   ...
   + For the minimum amount of 1,000,000 `SQD` you'll get enough bandwidth for most tasks. More details in [Token requirements and compute units](#staking-requirements-and-compute-units) section to estimate the required sum.
...
```diff
+  .setPortal({
+    url: 'http://127.0.0.1:8000/datasets/ethereum-mainnet',
+    http: {
...
```diff
+  .setPortal({
+    url: 'http://127.0.0.1:8000/datasets/solana-mainnet',
+    http: {
+      retryAttempts: Infinity
+    }
+  })
...
The minimum token locking requirement for a portal is 1,000,000 `SQD`. This should be enough for most workloads. If you want to know the exact capabilities this gives you, read on.
