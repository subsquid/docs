...
```
http://localhost:8000/datasets/ethereum-mainnet
```
...
Each locked `SQD` generates one spendable CU once per [epoch](/subsquid-network/faq/#epoch). Therefore, if your query needs X CUs once per Y epochs, you need to lock up at least `X/Y` `SQD`.
...
* When ran once every 60 minutes or 3 epochs at 20 min per epoch, such queries would require roughly `1500 / 3 = 500` `SQD` locked up.
