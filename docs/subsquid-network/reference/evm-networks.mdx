---
sidebar_position: 10
description: >-
  Public EVM datasets
title: EVM networks
---

export const FromBlockTooltip = ({children, tip}) => (
  <div class="group relative">
    <div class={`pointer-events-auto absolute -top-9 -left-3 w-max opacity-0 transition-opacity group-hover:opacity-100`}>
      <div>from block</div>
      <div>{tip}</div>
    </div>
    <span class="underline decoration-dotted">{children}</span>
  </div>
)

export const MoonbeamTooltip = ({children, tip}) => (
  <div class="group relative">
    <div class={`pointer-events-auto absolute -top-16 -left-3 w-max opacity-0 transition-opacity group-hover:opacity-100`}>
      <div>up to block</div>
      <div>5327886</div>
      <div><a href="https://github.com/moonbeam-foundation/moonbeam/issues/2626">learn more</a></div>
    </div>
    <span class="underline decoration-dotted">{children}</span>
  </div>
)

export const BitgertTooltip = ({children, tip}) => (
  <div class="group relative">
    <div class={`pointer-events-auto absolute -top-9 -left-3 w-max opacity-0 transition-opacity group-hover:opacity-100`}>
      <div>up to block</div>
      <div>4298094</div>
    </div>
    <span class="underline decoration-dotted">{children}</span>
  </div>
)

# EVM datasets

## From the open private network

This page lists the public EVM datasets available from the [open private network](/subsquid-network/overview/#open-private-network). If you are developing a squid, use the `lookupArchive` function available from [@subsquid/archive-registry](/subsquid-network/reference/registry) to get dataset URLs to be used with the [`setGateway()`](/sdk/reference/processors/evm-batch/general/#set-gateway) `EvmBatchProcessor` configuration method.

| Network                       | State diffs | Traces | Lookup command                                      |
|:-----------------------------:|:-----------:|:------:|:---------------------------------------------------:|
| arbitrum                      |             | ✓      | `lookupArchive('arbitrum')`                         |
| arbitrum-nova                 |             | ✓      | `lookupArchive('arbitrum-nova')`                    |
| arbitrum-sepolia              | ✓           | ✓      | `lookupArchive('arbitrum-sepolia')`                 |
| astar (*)                     |             |        | `lookupArchive('astar',` `{type: 'EVM'})`           |
| astar-zkevm-mainnet           |             |        | `lookupArchive('astar-zkevm-mainnet')`              |
| astar-zkyoto                  |             |        | `lookupArchive('astar-zkyoto')`                     |
| avalanche                     | ✓           | ✓      | `lookupArchive('avalanche')`                        |
| avalanche-testnet             | ✓           | ✓      | `lookupArchive('avalanche-testnet')`                |
| base-mainnet                  | ✓           | ✓      | `lookupArchive('base-mainnet')`                     |
| base-sepolia                  | ✓           | ✓      | `lookupArchive('base-sepolia')`                     |
| berachain-artio               |             |        | `lookupArchive('berachain-artio')`                  |
| binance                       | ✓           | ✓      | `lookupArchive('binance')`                          |
| binance-testnet               |             |        | `lookupArchive('binance-testnet')`                  |
| bitgert-mainnet               | <BitgertTooltip>?</BitgertTooltip> | ✓ | `lookupArchive('bitgert-mainnet')` |
| bitgert-testnet               |             | ✓      | `lookupArchive('bitgert-testnet')`                  |
| blast-l2-mainnet              | ✓           | ✓      | `lookupArchive('blast-l2-mainnet')`                 |
| blast-sepolia                 |             |        | `lookupArchive('blast-sepolia')`                    |
| core-mainnet                  |             | ✓      | `lookupArchive('core-mainnet')`                     |
| cyberconnect-l2-testnet       | ✓           | ✓      | `lookupArchive('cyberconnect-l2-testnet')`          |
| dfk-chain                     |             |        | `lookupArchive('dfk-chain')`                        |
| dogechain-mainnet             |             |        | `lookupArchive('dogechain-mainnet')`                |
| eth-holesky                   | ✓           | ✓      | `lookupArchive('eth-holesky')`                      |
| eth-mainnet                   | ✓           | ✓      | `lookupArchive('eth-mainnet')`                      |
| eth-sepolia                   | ✓           | ✓      | `lookupArchive('eth-sepolia')`                      |
| etherlink-testnet             |             |        | `lookupArchive('etherlink-testnet')`                |
| exosama                       | ✓           | ✓      | `lookupArchive('exosama')`                          |
| fantom                        |             |        | `lookupArchive('fantom')`                           |
| fantom-testnet                |             |        | `lookupArchive('fantom-testnet')`                   |
| flare-mainnet                 |             |        | `lookupArchive('flare-mainnet')`                    |
| gnosis-mainnet                | ✓           | ✓      | `lookupArchive('gnosis-mainnet')`                   |
| immutable-zkevm-mainnet       |             |        | `lookupArchive('immutable-zkevm-mainnet')`          |
| immutable-zkevm-testnet       |             |        | `lookupArchive('immutable-zkevm-testnet')`          |
| linea-mainnet                 |             | ✓      | `lookupArchive('linea-mainnet')`                    |
| mantle-mainnet                |             |        | `lookupArchive('mantle-mainnet')`                   |
| mantle-sepolia                |             |        | `lookupArchive('mantle-sepolia')`                   |
| metis-mainnet                 |             |        | `lookupArchive('metis-mainnet')`                    |
| moonbase (*)                  |             |        | `lookupArchive('moonbase',` `{type: 'EVM'})`        |
| moonbeam (*)                  |             | <MoonbeamTooltip>?</MoonbeamTooltip> | `lookupArchive('moonbeam',` `{type: 'EVM'})` |
| moonriver (*)                 |             | ✓      | `lookupArchive('moonriver',` `{type: 'EVM'})`       |
| neon-devnet (!)               |             |        | `lookupArchive('neon-devnet')`                      |
| neon-mainnet                  |             |        | `lookupArchive('neon-mainnet')`                     |
| opbnb-mainnet                 |             |        | `lookupArchive('opbnb-mainnet')`                    |
| opbnb-testnet                 |             |        | `lookupArchive('opbnb-testnet')`                    |
| optimism-mainnet              | <FromBlockTooltip tip="105235063">✓</FromBlockTooltip> | ✓ | `lookupArchive('optimism-mainnet')` |
| optimism-sepolia              |             |        | `lookupArchive('optimism-sepolia')`                 |
| peaq-mainnet                  |             |        | `lookupArchive('peaq-mainnet',` `{type: 'EVM'})`    |
| polygon                       |             |        | `lookupArchive('polygon')`                          |
| polygon-amoy-testnet          |             |        | `lookupArchive('polygon-amoy-testnet')`             |
| polygon-zkevm                 |             |        | `lookupArchive('polygon-zkevm')`                    |
| polygon-zkevm-cardona-testnet |             |        | `lookupArchive('polygon-zkevm-cardona-testnet')`    |
| prom-testnet                  |             |        | `lookupArchive('prom-testnet')`                     |
| puppynet                      |             |        | `lookupArchive('puppynet')`                         |
| scroll-mainnet                |             | ✓      | `lookupArchive('scroll-mainnet')`                   |
| scroll-sepolia                |             | ✓      | `lookupArchive('scroll-sepolia')`                   |
| shibuya-testnet (*)           |             |        | `lookupArchive('shibuya-testnet',` `{type: 'EVM'})` |
| shiden-mainnet (*)            |             |        | `lookupArchive('shiden-mainnet')`                   |
| skale-nebula                  |             |        | `lookupArchive('skale-nebula')`                     |
| tanssi                        |             |        | `lookupArchive('tanssi')`                           |
| x1-testnet                    |             |        | `lookupArchive('x1-testnet')`                       |
| xlayer-mainnet                |             |        | `lookupArchive('xlayer-mainnet')`                   |
| zksync-mainnet                |             | <FromBlockTooltip tip="15500000">✓</FromBlockTooltip> | `lookupArchive('zksync-mainnet')` |
| zksync-sepolia                |             | ✓      | `lookupArchive('zksync-sepolia')`                   |
| zora-mainnet                  | ✓           | ✓      | `lookupArchive('zora-mainnet')`                     |

(*) Only for EVM data. For Substrate/ink! data use the corresponding [Substrate dataset](/subsquid-network/reference/substrate-networks).

(!) Genesis block for `neon-devnet` is 177455580.

### Examples

Registry lookup:
```typescript
import { lookupArchive } from '@subsquid/archive-registry'

const processor = new EvmBatchProcessor()
  // the lookupArchive() call resolves to
  // 'https://v2.archive.subsquid.io/network/ethereum-mainnet'
  .setGateway(lookupArchive('eth-mainnet'))
```
Explicit dataset endpoints:
```typescript
const processor = new EvmBatchProcessor()
  .setGateway('https://v2.archive.subsquid.io/network/ethereum-mainnet')
```

### Raw URLs

Here are the raw dataset URLs (retrieved 2024-04-16):

| Network                       | Dataset endpoint URL                                                 |
|:-----------------------------:|:--------------------------------------------------------------------:|
| arbitrum                      | https://v2.archive.subsquid.io/network/arbitrum-one                  |
| arbitrum-nova                 | https://v2.archive.subsquid.io/network/arbitrum-nova                 |
| arbitrum-sepolia              | https://v2.archive.subsquid.io/network/arbitrum-sepolia              |
| astar                         | https://v2.archive.subsquid.io/network/astar-mainnet                 |
| astar-zkevm-mainnet           | https://v2.archive.subsquid.io/network/astar-zkevm-mainnet           |
| astar-zkyoto                  | https://v2.archive.subsquid.io/network/astar-zkyoto                  |
| avalanche                     | https://v2.archive.subsquid.io/network/avalanche-mainnet             |
| avalanche-testnet             | https://v2.archive.subsquid.io/network/avalanche-testnet             |
| base-mainnet                  | https://v2.archive.subsquid.io/network/base-mainnet                  |
| base-sepolia                  | https://v2.archive.subsquid.io/network/base-sepolia                  |
| berachain-artio               | https://v2.archive.subsquid.io/network/berachain-artio               |
| binance                       | https://v2.archive.subsquid.io/network/binance-mainnet               |
| binance-testnet               | https://v2.archive.subsquid.io/network/binance-testnet               |
| bitgert-mainnet               | https://v2.archive.subsquid.io/network/bitgert-mainnet               |
| bitgert-testnet               | https://v2.archive.subsquid.io/network/bitgert-testnet               |
| blast-l2-mainnet              | https://v2.archive.subsquid.io/network/blast-l2-mainnet              |
| blast-sepolia                 | https://v2.archive.subsquid.io/network/blast-sepolia                 |
| core-mainnet                  | https://v2.archive.subsquid.io/network/core-mainnet                  |
| cyberconnect-l2-testnet       | https://v2.archive.subsquid.io/network/cyberconnect-l2-testnet       |
| dfk-chain                     | https://v2.archive.subsquid.io/network/dfk-chain                     |
| dogechain-mainnet             | https://v2.archive.subsquid.io/network/dogechain-mainnet             |
| eth-holesky                   | https://v2.archive.subsquid.io/network/ethereum-holesky              |
| eth-mainnet                   | https://v2.archive.subsquid.io/network/ethereum-mainnet              |
| eth-sepolia                   | https://v2.archive.subsquid.io/network/ethereum-sepolia              |
| etherlink-testnet             | https://v2.archive.subsquid.io/network/etherlink-testnet             |
| exosama                       | https://v2.archive.subsquid.io/network/exosama                       |
| fantom                        | https://v2.archive.subsquid.io/network/fantom-mainnet                |
| fantom-testnet                | https://v2.archive.subsquid.io/network/fantom-testnet                |
| flare-mainnet                 | https://v2.archive.subsquid.io/network/flare-mainnet                 |
| gnosis-mainnet                | https://v2.archive.subsquid.io/network/gnosis-mainnet                |
| immutable-zkevm-mainnet       | https://v2.archive.subsquid.io/network/immutable-zkevm-mainnet       |
| immutable-zkevm-testnet       | https://v2.archive.subsquid.io/network/immutable-zkevm-testnet       |
| linea-mainnet                 | https://v2.archive.subsquid.io/network/linea-mainnet                 |
| mantle-mainnet                | https://v2.archive.subsquid.io/network/mantle-mainnet                |
| mantle-sepolia                | https://v2.archive.subsquid.io/network/mantle-sepolia                |
| metis-mainnet                 | https://v2.archive.subsquid.io/network/metis-mainnet                 |
| moonbase                      | https://v2.archive.subsquid.io/network/moonbase-testnet              |
| moonbeam                      | https://v2.archive.subsquid.io/network/moonbeam-mainnet              |
| moonriver                     | https://v2.archive.subsquid.io/network/moonriver-mainnet             |
| neon-devnet                   | https://v2.archive.subsquid.io/network/neon-devnet                   |
| neon-mainnet                  | https://v2.archive.subsquid.io/network/neon-mainnet                  |
| opbnb-mainnet                 | https://v2.archive.subsquid.io/network/opbnb-mainnet                 |
| opbnb-testnet                 | https://v2.archive.subsquid.io/network/opbnb-testnet                 |
| optimism-mainnet              | https://v2.archive.subsquid.io/network/optimism-mainnet              |
| optimism-sepolia              | https://v2.archive.subsquid.io/network/optimism-sepolia              |
| peaq-mainnet                  | https://v2.archive.subsquid.io/network/peaq-mainnet                  |
| polygon                       | https://v2.archive.subsquid.io/network/polygon-mainnet               |
| polygon-amoy-testnet          | https://v2.archive.subsquid.io/network/polygon-amoy-testnet          |
| polygon-zkevm                 | https://v2.archive.subsquid.io/network/polygon-zkevm-mainnet         |
| polygon-zkevm-cardona-testnet | https://v2.archive.subsquid.io/network/polygon-zkevm-cardona-testnet |
| prom-testnet                  | https://v2.archive.subsquid.io/network/prom-testnet                  |
| puppynet                      | https://v2.archive.subsquid.io/network/puppynet                      |
| scroll-mainnet                | https://v2.archive.subsquid.io/network/scroll-mainnet                |
| scroll-sepolia                | https://v2.archive.subsquid.io/network/scroll-sepolia                |
| shibuya-testnet               | https://v2.archive.subsquid.io/network/shibuya-testnet               |
| shiden-mainnet                | https://v2.archive.subsquid.io/network/shiden-mainnet                |
| skale-nebula                  | https://v2.archive.subsquid.io/network/skale-nebula                  |
| tanssi                        | https://v2.archive.subsquid.io/network/tanssi                        |
| x1-testnet                    | https://v2.archive.subsquid.io/network/x1-testnet                    |
| xlayer-mainnet                | https://v2.archive.subsquid.io/network/xlayer-mainnet                |
| zksync-mainnet                | https://v2.archive.subsquid.io/network/zksync-mainnet                |
| zksync-sepolia                | https://v2.archive.subsquid.io/network/zksync-sepolia                |
| zora-mainnet                  | https://v2.archive.subsquid.io/network/zora-mainnet                  |
