...
Type-safe access and decoding of the call and event data is done with the help of the classes generated by a suitable kind of the [typegen tool](/basics). 
...
The snippet below assumes that we are using `TypeormDatabase` and that the database schema together with the model entities types has already been prepared. Consult [/basics/schema-spec] for mode details.
