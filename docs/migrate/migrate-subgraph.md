...
We could get the data transformed and saved by [using the existing handlers](/basics/batch-processing/#migrate-from-handlers), but then the gravatars would be processed one by one and the syncing process would be slow. Instead, here we restructure the handlers' code to transform the event data into `Gravatar` objects in batches.

`EvmBatchProcessor` receives an ordered batch of event items it is subscribed to (which can be inspected in the output). The batch typically consists of a mix of event and transaction data. In our case we have only two logs -- emitted when a gravatar is created and updated. We start with an auxiliary function that extracts and normalizes the event data from the logs using the helper classes generated by `evm-codegen` from the ABI.
