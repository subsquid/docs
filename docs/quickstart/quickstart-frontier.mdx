...
Inspect `src/processor.ts` and set the network in the [processor configuration step](/develop-a-squid/substrate-processor/configuration).
Set `RPC_ENDPOINT` in `.env` to a Websocket endpoint of the corresponding network.
...
### Step 6: Create the database schema and run the processor
