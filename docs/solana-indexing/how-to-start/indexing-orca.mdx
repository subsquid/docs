...
Checking out the generated `src/abi/whirlpool.ts` file we see all events and contract functions listed in the IDL. Among them there is the `swap` instruction:
```typescript
