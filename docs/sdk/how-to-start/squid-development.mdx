...
```
1. Launch a PostgreSQL container with `docker compose up -d`
2. Start the squid processor with `sqd process`. You should see output that contains lines like these ones:
   ...
   ```
3. Start the GraphQL server by running `npx squid-graphql-server` in a separate terminal, then visit the [GraphiQL console](http://localhost:4350/graphql) to verify that the GraphQL API is up.
...
1. Use a data source appropriate for your chain and task.
   - It is possible to [use RPC](/sdk/reference/processors/evm-batch/general/#set-rpc-endpoint) as the only data source, but [adding](/sdk/reference/processors/evm-batch/general/#set-gateway) a [Subsquid Network](/subsquid-network/reference/evm-networks) data source will make your squid sync much faster.
