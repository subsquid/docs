...
6. [Generate utility classes](/sdk/reference/typegen/generation) for decoding [USDT contract](https://etherscan.io/address/0xdac17f958d2ee523a2206206994597c13d831ec7) data based on its ABI downloaded from Etherscan:
   ```bash
   npx \
   squid-evm-typegen \
   src/abi \
...
7. Tie all the generated code together with a `src/main.ts` executable with the following code blocks:
   ...
   * [`TypeormDatabase`](/sdk/reference/store/typeorm) object definition
     ...
     ```
   * A call to [`processor.run()`](/sdk/resources/processor/architecture/#processorrun) with an inline definition of the [batch handler](/sdk/resources/processor/architecture/#batch-context)
     ```ts
