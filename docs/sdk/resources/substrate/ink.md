...
Generate and use the facade classes for decoding ink! smart contract data as described in the [typegens reference](/sdk/resources/tools/typegen). You can also make [direct contract state queries](/sdk/reference/typegen/state-queries/?typegen=ink) using the `Contract` class generated by `squid-ink-typegen`.
