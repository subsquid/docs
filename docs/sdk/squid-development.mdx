...
```
See also the [EVM data decoding reference](/dead).
...
See also the [Substrate data decoding reference](/dead).

You can also decode the data of certain pallet-specific events and transactions with specialized tools:
 - use the utility classes made with `@substrate/squid-ink-typegen` to [decode events emitted by ink! contracts](/sdk/resources/substrate/ink/#ink-typegen)
 - use the [`@subsquid/frontier` utils](/sdk/resources/substrate/frontier-evm/#event-and-transaction-data-parsing) and the [EVM typegen](/dead) to decode event logs and transactions of EVM contracts
...
4. Ensure that the squid has access to a blank database. The easiest way to do so is to start PostgreSQL in a Docker container with
   ...
   The alternative is to use an [external database](/sdk/resources/persisting-data/typeorm/#using-an-external-database).
...
```
The store will write the files automatically as soon as the buffer reaches the size set by the `chunkSizeMb` field of the `Database` constructor argument, or at the end of the batch if a call to [`setForceFlush()`](/dead) was made anywhere in the batch handler.
...
At low output data rates (e.g. if your entire dataset is in tens of Mbytes or under) take care to call [`ctx.store.setForceFlush()`](/dead) when appropriate to make sure your data actually gets written.
