...
1. How the data should be delivered. Options:
   - [PostgreSQL](/sdk/resources/store/typeorm) with an optional [GraphQL API](/sdk/reference/graphql-server) - can be real-time
   - [file-based dataset](/sdk/resources/store/file) - local or on S3
...
3. What are the technologies powering the blockchain(s) in question. Supported options:
   ...
   Note that you can use Subsquid via [RPC ingestion](/sdk/resources/processor/unfinalized-blocks) even if your network is not listed.
4. What exact data should be retrieved from blockchain(s)
5. Whether you need to mix in any [off-chain data](/sdk/resources/processor/external-api)
...
4. The required data is available from `Transfer` events emitted by the `Balances` pallet. Take a look at our [Substrate data sourcing miniguide](/sdk/resources/processor/substrate/data-sourcing-miniguide) for more info on how to figure out which pallets, events and calls are necessary for your task.
5. No off-chain data will be necessary for this task.
...
- A minimal template intended for developing EVM squids. Indexes ETH burns.
  ...
  ```
- Classic [example Subgraph](https://github.com/graphprotocol/example-subgraph) after a [migration](/sdk/resources/integration/migrate/migrate-subgraph) to Subsquid.
  ```bash
  sqd init my-squid-name -t gravatar
  ```
- A template showing how to [combine data from multiple chains](/sdk/resources/integration/multichain). Indexes USDC transfers on Ethereum and Binance.
  ```bash
...
<TabItem value="evm" label="EVM">
```
Retrieve JSON ABIs for all contracts of interest (e.g. from Etherscan), taking care to get implementation ABIs for [proxies](/sdk/resources/processor/evm/proxy-contracts) where appropriate. Assuming that you saved the ABI files to `./abi`, you can then regenerate the utilities with
```bash
sqd typegen
```
...
See also the [EVM typegen code generation](/sdk/reference/typegen/generation/?typegen=evm) reference page.
...
* [Substrate typegen configuration](/sdk/reference/typegen/generation/?typegen=substrate)
* [ink! typegen configuration](/sdk/reference/typegen/generation/?typegen=ink)

<details>
<summary>A tip for `frontier-evm` squids</summary>

These squids use both Substrate typegen _and_ EVM typegen. To generate all the required utilities, [configure the Substrate part](/sdk/reference/typegen/generation/?typegen=substrate), then save all relevant JSON ABIs to `./abi`, then run
```bash
sqd typegen
```
...
1. Use a [data source](/sdk/reference/processors/evm-batch/general/#set-data-source) appropriate for your chain and task.
   - It is possible to use RPC as the only data source, but adding a [Subsquid Network](/subsquid-network/reference/evm-networks) data source will make your squid sync much faster.
   - RPC is a hard requirement if you're building a real-time API.
   - If you're using RPC as one of your data sources, make sure to [set the number of finality confirmations](/sdk/reference/processors/evm-batch/general/#set-finality-confirmation) so that [hot blocks ingestion](/sdk/resources/processor/unfinalized-blocks) works properly.
...
1. Use a [data source](/sdk/reference/processors/substrate-batch/general/#set-data-source) appropriate for your chain and task
...
3. If your squid indexes any of the following:
   - an [ink! contract](/sdk/resources/processor/substrate/ink)
   - an EVM contract running on the [Frontier EVM pallet](/sdk/resources/processor/substrate/frontier-evm)
   - [Gear messages](/sdk/resources/processor/substrate/gear)
...
In templates, the batch handler is defined at the [`processor.run()`](/sdk/resources/processor/architecture/#processorrun) call in `src/main.ts` as an inline function. Its sole argument `ctx` contains:
...
- at `ctx._chain`: the means to access RPC for [state calls](#extras-state-calls)

This structure ([reference](/sdk/resources/processor/architecture/#batch-context)) is common for all processors; the structure of `ctx.blocks` items varies.
...
```
See also the [EVM data decoding reference](/sdk/reference/typegen/decoding/?typegen=evm).
...
See also the [Substrate data decoding reference](/sdk/reference/typegen/decoding/?typegen=substrate).

You can also decode the data of certain pallet-specific events and transactions with specialized tools:
 - use the utility classes made with `@substrate/squid-ink-typegen` to [decode events emitted by ink! contracts](/sdk/reference/typegen/decoding/?typegen=ink)
 - use the [`@subsquid/frontier` utils](/sdk/reference/frontier) and the [EVM typegen](/sdk/reference/typegen/decoding/?typegen=evm) to decode event logs and transactions of EVM contracts
...
If you need external (i.e. non-blockchain) data in your transformation, take a look at the [External APIs and IPFS](/sdk/resources/processor/external-api) page.

If any of the on-chain data you need is unavalable from the processor or incovenient to retrieve with it, you have an option to get it via [direct chain queries](/sdk/reference/typegen/state-queries).

### V. Prepare the store {#store}

At `src/main.ts`, change the [`Database`](/sdk/resources/store/overview) object definition to accept your output data. The methods for saving data will be exposed by `ctx.store` within the [batch handler](/sdk/resources/processor/architecture).
...
See the `typeorm-store` [guide](/sdk/resources/store/typeorm) and [reference](/sdk/reference/store/typeorm) for more info.
...
```
The store will write the files automatically as soon as the buffer reaches the size set by the `chunkSizeMb` field of the `Database` constructor argument, or at the end of the batch if a call to [`setForceFlush()`](/sdk/resources/store/file/#setforceflush) was made anywhere in the batch handler.

See the `file-store` [guide](/sdk/resources/store/file) and the [reference pages of its extensions](/sdk/reference/store/file).
...
Follow the [guide](/sdk/resources/store/bigquery).
...
See also the [patterns](/sdk/resources/integration/batch-processing/#patterns) and [anti-pattens](/sdk/resources/integration/batch-processing/#anti-patterns) sections of the Batch processing guide.
...
At low output data rates (e.g. if your entire dataset is in tens of Mbytes or under) take care to call [`ctx.store.setForceFlush()`](/sdk/resources/store/file/#setforceflush) when appropriate to make sure your data actually gets written.
...
* Deploy your squid [on own infrastructure](/sdk/resources/integration/self-hosting) or to [Subsquid Cloud](/cloud)
