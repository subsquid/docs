...
Let's started by creating a new squid ETL, so in a terminal, launch the command:
...
Next, let's created a new file, named `tables.ts`, in case the project should be expanded to multiple files. This is where it's possible to provide a filename for the CSV files, as well as configure their data structure, the same way as if they were a database table (the class name is no coincidence):
...
It's then possible to decode the event and prepare an object with the right data structure, which is added to an array.

Outside both loops, I perform a single operation to save all the objects in the previously prepared array. This batch operation helps by increasing the overall performance.
