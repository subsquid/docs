...
* **`@index`** - signals thatt his field should be indexed by the database. Very useful for incresing performance on fields queried often.
...
First, it is necessary to obtain the definition of the Application Binary Interface (ABI). This can be obtained in the form of a JSON file, which will be imported into the project. In the context of this example, you can find the Exosama collection smart contract (a modified ERC-721) at [this address](https://github.com/RaekwonIII/eth-workshop/raw/master/src/abi/exo.json).
